...
export default function AboutFeatures() {
  ...
  return (
    <section className="w-full py-16 space-y-24">
      {features.map((f, i) => (
        <div key={i} className="w-full grid md:grid-cols-2 gap-10 items-center">
          {/* Image */}
          <div
          ...
          <div className={`${i % 2 === 1 ? "md:order-1" : ""}`}>
            ...
            <h3 className="text-2xl md:text-3xl text-color font-bold mb-4">
              {f.title}
            </h3>
            <p className=" mb-5 text-color-soft">{f.desc}</p>

            <ul className="mb-5 space-y-2 text-color-soft">
              {f.bullets.map((b, j) => (
            ...
            <div className="flex flex-wrap gap-3">
              {f.buttons.map((btn, k) => (
                <a
                  key={k}
                  href={btn.href}
                  className={`px-5 py-2 rounded-md text-sm font-medium hover:shadow-md transition border-none ${
                    k === 0
                      ? "background-color-secondary text-white"
